...
function populateFilters(data) {
	const batches = uniqueValues((record) => record.batch).sort((a, b) => compareBatchLabels(a, b));
	...
	tagSelect.innerHTML = tags
		.map(({ tag, count }) => `<option value="${tag}">${tag} (${count.toLocaleString()})</option>`)
		.join('');
}
...
function countTag(data, tag) {
	return data.reduce((total, record) => total + (record.tags?.includes(tag) ? 1 : 0), 0);
}
...
function buildCompanyCard(record) {
	...
	fragment.querySelector('.company-team').textContent = record.team_size ?? 'Unknown';
	fragment.querySelector('.company-year').textContent = record.year_founded ?? '—';
...
function applyFilters() {
	...
	console.log('[YC Explorer] Applying filters', JSON.stringify({ searchQuery, batches, tags }));
	filtered = companies.filter((record) => recordMatches(record, searchQuery, batches, tags));
	sortFilteredRecords();
...
function populateFounders(fragment, record) {
	...
	const details = record.founder_details?.filter((founder) => founder?.name || founder?.bio);
	...
	if (details?.length) {
		...
		details.forEach((founder) => {
			...
			const name = document.createElement(founder.linkedin_url ? 'a' : 'span');
